...
def test_model_optimized(model, test_loader, criterions, writer, save_dir, device, tasks, prob_threshold,
                         visualize_gradcam=False, save_gradcam_images=False, gradcam_task=None, colormap='hot'):
    """
    Enregistre TOUT sous --save_dir :
      - save_dir/metrics_test.json : métriques détaillées (acc, prec, rec, f1, AUC macro/micro & par classe)
      - save_dir/metrics_test_summary.csv : résumé lisible (par tâche)
      - save_dir/roc/<task>_roc.png : courbe(s) ROC avec NOMS DE CLASSES si calculables
      - save_dir/roc/<task>_roc_data.csv : FPR/TPR (micro + par classe valides) avec noms de classes
      - save_dir/gradcam/*.png : Grad-CAM (si demandé), concat (original | overlay) avec nom de classe si dispo
    """
    import os, time, json, numpy as np, torch, torch.nn as nn, cv2, csv, matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    from sklearn.preprocessing import label_binarize
    from sklearn.metrics import (
    ...
    if visualize_gradcam:
        ...
        if cam_task not in tasks:
            ...
            if last_conv_layer is None:
                print("[GradCAM] Aucune Conv2d trouvée dans feature_extractor -> CAM désactivé.")
                visualize_gradcam = False
            else:
    ...
    for batch_idx, (inputs, labels) in enumerate(test_loader):
        ...
        # ----- Grad-CAM (sauvegarde dans save_dir/gradcam) -----
        if visualize_gradcam:
            ...
            for i in range(batch_size):
                ...
                if save_gradcam_images:
                    out_name = f"gradcam_b{batch_idx:04d}_i{i:03d}_task-{cam_task}_class-{cls_idx}-{cls_name}.png"
                    out_path = os.path.join(gradcam_dir, out_name)
                    _hstack_and_save(orig_rgb01, overlay, out_path)

    # ---------- Métriques + AUC/ROC (sauvegardes) ----------
    average_loss = (total_loss / total_samples) if total_samples else 0.0
    metrics = {}
    summary_rows = []  # pour metrics_test_summary.csv

    for task, class_list in tasks.items():
        class_names = list(class_list)  # garantir l'ordre et l'indexation
        ...
        probs_arr  = np.array(all_probs_list[task], dtype=float) if len(all_probs_list[task]) else np.zeros((0, len(class_names)), dtype=float)

        # Scores discrets (on enlève Unknown = -1)
        valid_mask = (preds_arr != -1)
        if valid_mask.sum() > 0:
            acc  = float(np.mean(preds_arr[valid_mask] == labels_arr[valid_mask]))
            prec = float(precision_score(labels_arr[valid_mask], preds_arr[valid_mask], average='weighted', zero_division=0))
            rec  = float(recall_score( labels_arr[valid_mask], preds_arr[valid_mask], average='weighted', zero_division=0))
            f1   = float(f1_score(     labels_arr[valid_mask], preds_arr[valid_mask], average='weighted', zero_division=0))
        ...
        # on (re)crée le CSV et on y mettra: type, class_name, fpr, tpr
        with open(roc_csv_path, "w", newline="") as fcsv:
            ...
            if probs_arr.shape[0] > 0:
                ...
                if len(present_classes) >= 2 and probs_valid.shape[1] == n_classes:
                    if n_classes == 2:
                        # binaire: classe positive = index 1 -> nom class_names[1]
                        if set([0,1]).issubset(set(present_classes)) and probs_valid.shape[1] >= 2:
                            ...
                            plt.tight_layout(); plt.savefig(roc_fig_path, dpi=150); plt.close()
                    else:
                        # Multiclasse
                        y_bin_full = label_binarize(y_true, classes=list(range(n_classes)))  # (N,C)
        ...
        metrics[task] = {
            ...
            'class_names': class_names,  # ajouté pour traçabilité
        }
        ...
        if auc_macro is not None: msg += f", AUC_macro={auc_macro:.4f}"
        if auc_micro is not None: msg += f", AUC_micro={auc_micro:.4f}"
        print(msg)
    ...
    # ---------- Sauvegardes obligatoires sous --save_dir ----------
    metrics_out = {
    ...
    with open(csv_path, "w", newline="") as fcsv:
        fieldnames = ["task","accuracy","precision","recall","f1_score","auc_macro","auc_micro","roc_png"]
